...
# [6, 5, 4, 4]
result = [len(names[i]) for i in range(4)]
print(result)

# 길이가 5 이상인 이름만 뽑기
result = [names[i] for i in range(4) if len(names[i]) >= 5]
print(result)
...
result = [scores[i] for i in range(8) if scores[i] >= 60]
print(result)                       # ✅ [85, 73, 90, 68, 100] 출력


# 2️⃣ 60점 이상인 경우 "합격", 60점 미만은 "불합격"으로 처리
result = ["합격" if scores[i] >= 60 else "불합격" for i in range(8)]
